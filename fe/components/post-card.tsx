...
export function PostCard({ post }: { post: Post }) {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.error || t.compSocial.failed;
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      ...
      toast.success(t.compSocial.linkCopied);
    }
  };
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <Link href={`/c/${post.creator?.username || ""}`} className="hover:underline">
                <span className="font-semibold text-sm">{post.creator?.display_name || t.compSocial.creator}</span>
              </Link>
      ...
      <CardContent className="pt-0">
        {isLocked ? (
          <div className="rounded-2xl bg-gradient-to-br from-primary-50 to-primary-100/50 dark:from-blue-900/20 dark:to-blue-800/10 border border-primary-200/50 dark:border-blue-800/30 p-8 text-center">
            ...
            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{t.compSocial.paidContent}</p>
            {error && <p className="mt-2 text-xs text-red-600">{error}</p>}
            <div className="mt-4">
              {error.includes("Top-up") || error.includes("Credit") ? (
                <Link href="/wallet/topup"><Button size="sm">{t.compSocial.topupCredit}</Button></Link>
              ) : (
                <Button size="sm" onClick={handleBuy} disabled={buying}>
                  {buying ? t.compSocial.processing : interpolate(t.compSocial.buy, { price: formatCredit(post.price || 0) })}
                </Button>
              )}
            </div>
          </div>
            ...
            {comments.map((c: any) => (
              <div key={c.id} className="flex gap-2.5 text-sm">
                ...
                <div className="bg-primary-50/50 dark:bg-navy-800 rounded-2xl px-3 py-2">
                  <span className="font-semibold text-xs">{c.user?.display_name || t.compSocial.user}</span>
                  <p className="text-gray-600 dark:text-gray-400 text-xs mt-0.5">{c.content}</p>
                </div>
              </div>
            ))}
            <div className="flex gap-2">
              <Input placeholder={t.compSocial.writeComment} value={commentText} onChange={(e) => setCommentText(e.target.value)}
