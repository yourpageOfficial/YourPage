...
export default function DashboardDonations() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-display font-black tracking-tight mb-5">{t.dashboardCore.donationsReceived}</h1>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <Card className="bg-gradient-to-br from-pink-50 to-white dark:from-pink-900/10 dark:to-navy-800">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-[10px] text-gray-400 uppercase tracking-wider mt-0.5">{t.dashboardCore.totalDonations}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-black text-green-500">{formatCredit(totalNet)}</p>
            <p className="text-[10px] text-gray-400 uppercase tracking-wider mt-0.5">{t.dashboardCore.netReceived}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-black">{uniqueDonors}</p>
            <p className="text-[10px] text-gray-400 uppercase tracking-wider mt-0.5">{t.dashboardCore.donors}</p>
          </CardContent>
        </Card>
      </div>
      ...
      {donations && donations.length > 0 ? (
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-semibold">{d.is_anonymous ? t.dashboardCore.anonymous : (d.supporter?.display_name || d.donor_name)}</p>
                      <p className="text-[10px] text-gray-400">{new Date(d.created_at).toLocaleDateString(locale === "id" ? "id-ID" : "en-US", { day: "numeric", month: "short", year: "numeric" })}</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between mt-2 text-[10px] text-gray-400">
                  <span>{t.dashboardCore.net} <span className="text-green-500 font-bold">{formatCredit(d.net_amount_idr)}</span></span>
        ...
        <Card><CardContent className="p-12 text-center">
          ...
          <p className="font-semibold">{t.dashboardCore.noDonations}</p>
          <p className="text-sm text-gray-400 mt-1">{t.dashboardCore.noDonationsDesc}</p>
        </CardContent></Card>
      )}
    </div>
  );
