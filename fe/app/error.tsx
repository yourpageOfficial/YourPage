"use client";

import { AlertTriangle } from "lucide-react";
import { useTranslation } from "@/lib/internationalization";

export default function Error({ reset }: { error: Error; reset: () => void }) {
  const { t } = useTranslation();
  return (
    <div style={{ minHeight: "50vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", textAlign: "center" }}>
      <div>
        <AlertTriangle style={{ width: "3rem", height: "3rem", marginBottom: "1rem" }} aria-hidden="true" />
        <h2 style={{ fontSize: "1.25rem", fontWeight: "bold", marginBottom: "0.5rem" }}>{t.common.errorTitle}</h2>
        <p style={{ color: "#6B7280", fontSize: "0.875rem", marginBottom: "1.5rem" }}>{t.common.errorDesc}</p>
        <button onClick={reset} style={{ padding: "0.5rem 1.5rem", borderRadius: "0.75rem", background: "#EC4899", color: "white", border: "none", cursor: "pointer", fontWeight: 600 }}>
          {t.common.refresh}
        </button>
      </div>
    </div>
  );
}
