...
export default function AdminSettings() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-settings"] }); toast.success(t("saved")); },
  });
  ...
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>{t("config_title")}</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm font-medium">{t("platform_fee")}</label>
              <Input type="number" value={fee} onChange={(e) => setFee(e.target.value)} />
            </div>
            <div>
              <label className="text-sm font-medium">{t("min_withdrawal")}</label>
              <Input type="number" value={minWd} onChange={(e) => setMinWd(e.target.value)} />
            </div>
            <div>
              <label className="text-sm font-medium">{t("credit_rate")}</label>
        ...
        <Card>
          <CardHeader><CardTitle>{t("qris_title")}</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-gray-500 dark:text-gray-400">{t("qris_desc")}</p>
            {qrisPreview ? (
              <img src={qrisPreview} alt="QRIS" className="max-h-64 rounded border mx-auto" />
            ) : (
              <div className="h-48 bg-gray-100 dark:bg-gray-800 rounded flex items-center justify-center text-gray-400 dark:text-gray-500">{t("no_qris")}</div>
            )}
            ...
            <Button variant="outline" className="w-full" onClick={() => qrisRef.current?.click()} disabled={uploadQris.isPending}>
              <Upload className="mr-1 h-4 w-4" /> {uploadQris.isPending ? t("uploading") : t("upload_qris")}
            </Button>
          </CardContent>
        </Card>
      </div>
