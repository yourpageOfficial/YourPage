...
export default function CreatorPageView() {
  ...
  if (!creator) return <><Navbar /><div className="p-8 text-center text-gray-500 dark:text-gray-400">{t("not_found")}</div></>;
  ...
  const handleDonate = async () => {
    ...
    try {
      ...
      setDonateError(msg.includes("Credit") || msg.includes("insufficient") ? t("credit_insufficient") : msg);
    ...
    <>
      ...
      <main className="mx-auto max-w-5xl px-3 sm:px-4 py-6 sm:py-8">
        ...
        <div className="relative -mt-12 sm:-mt-14 mx-2 sm:mx-0">
          <Card className="p-4 sm:p-6">
            ...
            <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
              ...
              {creator.is_priority && <Badge className="text-xs bg-yellow-500 text-white shadow-md">⭐ {t("priority")}</Badge>}
            </div>
            ...
            <p className="mt-2 flex items-center justify-center sm:justify-start gap-1 text-sm text-gray-500 dark:text-gray-400">
              <Users className="h-4 w-4" /> {creator.follower_count} {t("followers_label")}
            </p>
            <div className="mt-3 flex items-center justify-center sm:justify-start gap-2">
              {isOwn ? (
                <>
                  <Link href="/dashboard/profile"><Button size="sm" variant="outline">{t("edit_profile")}</Button></Link>
                  <Button size="sm" variant="ghost" onClick={() => {
                    const url = window.location.href + "?ref=" + creator.username;
                    navigator.share ? navigator.share({ title: creator.display_name, url }) : navigator.clipboard.writeText(url).then(() => alert(t("referral_copied")));
                  }}>{t("share")}</Button>
                </>
              ) : user ? (
                <>
                  ...
                  <Link href={`/chat?creator=${creator.user_id}&price=${creator.chat_price_idr || 0}`}>
                    <Button size="sm" variant="outline">
                      💬 {t("chat_credit", { price: (creator.chat_price_idr ?? 0) > 0 ? (creator.chat_price_idr ?? 0) / 1000 : "" })}
                    </Button>
                  </Link>
                </>
              ) : (
                <Link href="/login"><Button size="sm" style={creator.page_color ? { backgroundColor: creator.page_color, color: 'white' } : undefined}>{t("follow")}</Button></Link>
              )}
            </div>
          </div>
            </div>
        ...
        <div className="mt-6 sm:mt-8 flex gap-1 border-b dark:border-gray-700 overflow-x-auto">
          {[
            { key: "posts", label: t("posts_tab", { count: posts?.length ?? 0 }) },
            { key: "catalog", label: t("catalog_tab", { count: products?.length ?? 0 }) },
          ].map(tb => (
            <button key={tb.key} onClick={() => setTab(tb.key as any)}
              className={`px-3 sm:px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${tab === tb.key ? "border-primary text-primary" : "border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"}`}
              style={tab === tb.key && creator.page_color ? { color: creator.page_color, borderColor: creator.page_color } : undefined}>
              {tb.label}
            </button>
          ))}
        </div>
        ...
        <div className="mt-6">
          {tab === "posts" && (
            <div className="space-y-4">
              ...
              {posts?.length === 0 && <p className="text-gray-500 text-sm">{t("no_posts")}</p>}
            </div>
          )}
          ...
          {tab === "catalog" && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              {products?.length === 0 && <p className="text-gray-500 text-sm col-span-full">{t("no_products")}</p>}
            </div>
          )}
        </div>
      </main>
              ...
              <Card className="shadow-xl border-2">
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="font-semibold">{tDonations("support_creator", { name: creator.display_name })}</p>
                    <button onClick={() => setShowDonate(false)} className="text-gray-400 hover:text-gray-600 dark:text-gray-400">✕</button>
                  </div>

                  {donateSuccess ? (
                    <div className="text-center py-2">
                      <p className="text-green-600 font-semibold">🎉 {tDonations("sent_success")}</p>
                      <Button size="sm" className="mt-2" onClick={() => setDonateSuccess(false)}>{tDonations("another")}</Button>
                    </div>
                  ) : (
                    <>
                      ...
                      <Input type="number" placeholder={tDonations("amount_other")} value={donateAmount} onChange={(e) => setDonateAmount(e.target.value)} className="h-8 text-sm" />
                      <Input placeholder={tDonations("message_optional")} value={donateMsg} onChange={(e) => setDonateMsg(e.target.value)} className="h-8 text-sm" maxLength={500} />
                      {donateError && <p className="text-xs text-red-600">{donateError}</p>}
                      {donateError.includes("Top-up") && <a href="/wallet/topup" className="text-xs text-primary hover:underline">{tDonations("topup_credit")}</a>}
                      <Button size="sm" className="w-full" onClick={handleDonate} disabled={donating || !donateAmount || parseInt(donateAmount) < 1000}
                        ...
                        {donating ? tDonations("try_again") : `${tDonations("send")} ${donateAmount ? formatIDR(parseInt(donateAmount)) : ""}`}
                      </Button>
                      <p className="text-[10px] text-gray-400 text-center">{tDonations("min_fee")}</p>
                    </>
                  )}
                </CardContent>
              </Card>
