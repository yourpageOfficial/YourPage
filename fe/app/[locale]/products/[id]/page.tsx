...
export default function ProductDetailPage() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (msg.includes("Credit") || msg.includes("insufficient")) setError(tDonations("credit_insufficient"));
        ...
        <div className="grid gap-6 sm:gap-8 md:grid-cols-5">
          ...
          <div className="md:col-span-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge className="text-xs" variant="outline">{product.delivery_type === "link" ? "🔗 " + t("link") : "📁 " + t("file")}</Badge>
              {!product.is_active && <Badge className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400">{tCommon("inactive")}</Badge>}
            </div>
            ...
            <p className="text-sm text-gray-500 mt-1">{product.sales_count} {t("sold")} · {formatDate(product.created_at)}</p>
            <div className="mt-2"><ReportButton targetType="product" targetId={id} /></div>

            {/* Description */}
            {product.description && (
              <div className="mt-6">
                <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">{t("description")}</h3>
            ...
            {product.assets?.length > 0 && (
              <div className="mt-6">
                <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">{t("files_included")}</h3>
                ...
                <Card className="border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="font-semibold text-green-700 dark:text-green-400">{t("already_purchased")}</p>
                    </div>
                    {downloadUrls.length > 0 ? (
                        ...
                        {downloadUrls.map((d: any, i: number) => (
                          <a key={d.asset_id || i} href={d.signed_url} target="_blank" rel="noopener">
                            <Button variant="outline" size="sm" className="w-full justify-start mb-1">
                              {d.file_name === "Link" ? "🔗 " + t("open_link") : <><Download className="mr-2 h-4 w-4" /> {d.file_name}</>}
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">{t("no_files")}</p>
                    )}
                  </CardContent>
                </Card>
              ) : error.includes("Credit") ? (
                <Link href="/wallet/topup">
                  <Button size="lg" className="w-full">{tDonations("topup_credit")}</Button>
                </Link>
              ) : (
                <Button size="lg" className="w-full" onClick={handleBuy} disabled={buying}>
                  <ShoppingCart className="mr-2 h-5 w-5" />
                  {buying ? tDonations("try_again") : `${t("buy")} — ${formatCredit(product.price_idr)}`}
                </Button>
              )}
            </div>
          </div>
