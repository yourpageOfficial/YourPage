  ...
  return (
    <AuthGuard>
      ...
      <main className="mx-auto max-w-2xl px-3 sm:px-4 py-6 sm:py-8">
        <h1 className="mb-2 text-xl sm:text-2xl font-bold">{t("title")}</h1>
        <p className="mb-4 text-sm text-gray-500 dark:text-gray-400">{t("subtitle")}</p>

        <div className="relative mb-6">
          <Search className="absolute left-3 top-2.5 h-4 w-4 text-gray-400 dark:text-gray-500" />
          <Input placeholder={t("search_placeholder")} value={query} onChange={(e) => setQuery(e.target.value)} className="pl-9" />
        </div>

        {!query && trending && trending.length > 0 && (
          <div className="mb-8">
            <h2 className="flex items-center gap-2 text-base font-semibold mb-3">
              <TrendingUp className="h-4 w-4 text-primary" /> {t("trending_creators")}
            </h2>
        ...
        {!query && creators && creators.length > 0 && (
          <div>
            <h2 className="text-base font-semibold mb-3">{t("all_creators")}</h2>
          ...
          <div>
            ...
            {results?.length === 0 && !isLoading && (
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-8">
                {t("no_results")} &ldquo;{query}&rdquo;. {t("try_another")}
              </p>
            )}
          </div>
        )}
