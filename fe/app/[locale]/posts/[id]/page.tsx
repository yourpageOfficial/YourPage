...
export default function PostDetailPage() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      setError(msg.includes("Credit") || msg.includes("insufficient") ? t("insufficient_credit") : msg);
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-3 sm:px-4 py-6 sm:py-8">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
          ...
          {post.access_type === "free" && <Badge>{tCommon("free")}</Badge>}
        </div>
        ...
        {post.is_locked ? (
          <Card className="mt-8">
            <CardContent className="p-10 text-center">
              ...
              <p className="mt-4 text-lg font-medium text-gray-700 dark:text-gray-300">{t("locked_title")}</p>
              <p className="mt-1 text-gray-500 dark:text-gray-400">{t("locked_desc")}</p>
              {error && <p className="mt-3 text-sm text-red-600">{error}</p>}
              <div className="mt-6 flex flex-col items-center gap-3">
                {error.includes("Credit") ? (
                  <Link href="/wallet/topup"><Button size="lg">{t("topup_now")}</Button></Link>
                ) : (
                  <Button size="lg" onClick={handleBuy} disabled={buying}>
                    {buying ? t("processing") : t("buy_btn", { price: formatCredit(post.price || 0) })}
                  </Button>
                )}
              </div>
            </CardContent>
                  ...
                  <div key={m.id} className="rounded-lg overflow-hidden">
                    ...
                    {m.media_type === "document" && (
                      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 text-center">
                        <p className="text-gray-500 dark:text-gray-400">{tProducts("document_attached")}</p>
                      </div>
                    )}
                  </div>
                ))}
        ...
        <div className="mt-8 flex items-center gap-6 py-4 border-t border-b">
          <button onClick={handleLike} className={`flex items-center gap-2 text-sm font-medium transition-colors ${liked ? "text-red-500" : "text-gray-500 hover:text-red-500"}`}>
            <Heart className={`h-5 w-5 ${liked ? "fill-current" : ""}`} /> {likeCount} {t("like")}
          </button>
          <button onClick={loadComments} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-primary transition-colors">
            <MessageCircle className="h-5 w-5" /> {post.comment_count || 0} {t("comment")}
          </button>
          <button onClick={() => { const url = window.location.href; navigator.share ? navigator.share({ title: post.title, url }) : navigator.clipboard.writeText(url).then(() => alert(t("link_copied"))); }}
        ...
        {showComments && (
          <div className="mt-6 space-y-4">
            {comments.length === 0 && <p className="text-sm text-gray-500 dark:text-gray-400">{t("no_comments")}</p>}
            {comments.map((c: any) => (
            ...
            <div className="flex gap-3 mt-4">
              <Input placeholder={t("write_comment")} value={commentText} onChange={(e) => setCommentText(e.target.value)}
