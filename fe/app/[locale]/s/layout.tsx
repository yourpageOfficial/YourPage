...
export default function SupporterLayout({ children }: { children: React.ReactNode }) {
  const t = useTranslations("SupporterNav");
  const pathname = usePathname();

  const items = [
    { href: "/s", label: t("feed"), icon: Rss },
    { href: "/s/posts", label: t("posts_collection"), icon: FileText },
    { href: "/s/products", label: t("products_collection"), icon: Package },
    { href: "/s/transactions", label: t("transactions"), icon: Receipt },
    { href: "/s/donations", label: t("my_donations"), icon: Heart },
    { href: "/s/wallet", label: t("wallet"), icon: Wallet },
    { href: "/s/settings", label: t("settings"), icon: Settings },
  ];
