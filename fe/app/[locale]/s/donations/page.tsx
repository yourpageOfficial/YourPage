...
export default function SupporterDonations() {
  const t = useTranslations("SupporterDonations");
  const { data } = useQuery({
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">{t("donations_sent")}</h1>
        <div className="text-right">
          <p className="text-sm text-gray-500 dark:text-gray-400">{t("total_donations")}</p>
      ...
      {data?.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="mt-3 text-gray-500 dark:text-gray-400">{t("no_donations")}</p>
          <Link href="/explore"><span className="text-sm text-primary hover:underline">{t("support_favorites")}</span></Link>
        </div>
      )}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <div>
                      <Link href={`/c/${d.creator?.username || ""}`} className="font-medium hover:text-primary">
                        {d.creator?.display_name || t("creator_fallback")}
                      </Link>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <span>·</span>
                      <span>{t("net_to_creator", { net: formatCredit(d.net_amount_idr) })}</span>
                      <span>·</span>
