      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <Card className="flex-1">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400">{t("credit_balance")}</p>
              <p className="text-lg font-bold">{formatCredit(wallet?.balance_credits ?? 0)}</p>
            </div>
            <Link href="/wallet/topup" className="ml-auto"><Button size="sm" variant="outline">{t("topup")}</Button></Link>
          </CardContent>
        </Card>
        <div className="flex gap-2">
          <Link href="/explore" className="flex-1"><Button variant="outline" className="w-full h-full"><Search className="mr-1 h-4 w-4" /> {t("explore")}</Button></Link>
          <Link href="/s/donations" className="flex-1"><Button variant="outline" className="w-full h-full"><Heart className="mr-1 h-4 w-4" /> {t("donations")}</Button></Link>
        </div>
      </div>

      <h2 className="text-lg font-bold mb-4">{t("feed_title")}</h2>
      {isLoading && <div className="space-y-4">{[1,2,3].map(i => <PostSkeleton key={i} />)}</div>}
      {data?.length === 0 && (
        <Card>
          <CardContent className="p-8 text-center">
            <p className="text-gray-500 dark:text-gray-400 mb-3">{t("empty_feed")}</p>
            <Link href="/explore"><Button>{t("explore_creators")}</Button></Link>
          </CardContent>
        </Card>
      )}
