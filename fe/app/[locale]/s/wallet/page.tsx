...
export default function SupporterWallet() {
  const t = useTranslations("SupporterWallet");
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/wallet/topup"><Button size="sm">{t("topup_btn")}</Button></Link>
      </div>
      <Card className="mb-6">
        <CardContent className="p-4">
          <p className="text-2xl sm:text-3xl font-bold text-primary">{t("balance_format", { balance: wallet?.balance_credits ?? 0 })}</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">{t("credit_rate")}</p>
        </CardContent>
      </Card>
