...
export default function SupporterProducts() {
  const t = useTranslations("SupporterProducts");
  const { data } = useQuery({
    ...
    <div>
      ...
      {data?.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="mt-3 text-gray-500 dark:text-gray-400">{t("no_products")}</p>
          <Link href="/explore"><Button variant="outline" size="sm" className="mt-3">{t("explore_creators")}</Button></Link>
        </div>
      )}
            ...
            <CardContent className="p-4">
              <div className="flex gap-4">
                ...
                <Link href={`/products/${p.id}`} className="shrink-0 self-center">
                  <Button size="sm"><Download className="mr-1 h-4 w-4" /> {t("download")}</Button>
                </Link>
              </div>
            </CardContent>
          </Card>
