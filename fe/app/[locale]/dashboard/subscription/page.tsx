...
export default function SubscriptionPage() {
  ...
  const subscribe = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("tier_updated"));
      qc.invalidateQueries({ queryKey: ["my-profile"] });
      fetchMe();
      setConfirmTier(null);
    },
    onError: (e: any) => toast.error(e.response?.data?.error || t("subscribe_failed")),
  });
    ...
    <div>
      ...
      {tierExpires && (
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          {t("active_until")} {new Date(tierExpires).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}
        </p>
      )}
            ...
            <Card key={t.id} className={`${isCurrent ? "border-primary ring-2 ring-primary/20" : ""}`}>
              <CardHeader className="pb-2">
                <div className="flex items-center gap-2">
                  ...
                  {isCurrent && <Badge>{tCommon("active")}</Badge>}
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  {t.price_idr === 0 ? (
                    ...
                    <p className="text-2xl font-bold">{formatCredit(t.price_idr)} <span className="text-sm font-normal text-gray-500 dark:text-gray-400">{t("credit_per_month")}</span></p>
                  )}
                </div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">{tCommon("fee")}: {t.fee_percent}% · Storage: {(t.storage_bytes / (1024*1024*1024)).toFixed(0)} GB</div>
                {isCurrent ? (
                  <Button disabled className="w-full">{t("current_plan")}</Button>
                ) : (
                  <Button className="w-full" variant={i === 1 ? "default" : "outline"} onClick={() => setConfirmTier(t)}>
                    {t.price_idr === 0 ? t("downgrade") : t("upgrade")}
                  </Button>
                )}
              </CardContent>
            </Card>
      ...
      <ConfirmDialog
        ...
        title={confirmTier?.price_idr === 0 ? t("downgrade_title") : t("upgrade_title", { name: confirmTier?.name })}
        description={confirmTier?.price_idr === 0
          ? t("downgrade_desc")
          : `${formatCredit(confirmTier?.price_idr || 0)} ${t("upgrade_desc")}`}
        confirmText={confirmTier?.price_idr === 0 ? t("confirm_downgrade") : t("confirm_upgrade")}
