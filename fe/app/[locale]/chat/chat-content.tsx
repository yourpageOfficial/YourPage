...
export default function ChatContent() {
  ...
  const send = useMutation({
    ...
    onError: (e: any) => toast.error(e.response?.data?.error || t("send_failed")),
  });
      ...
      <AuthGuard><Navbar />
        <div className="mx-auto max-w-2xl flex flex-col h-[calc(100vh-3.5rem-3.5rem)] sm:h-[calc(100vh-4rem)]">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4 space-y-2">
            {(!messages || messages.length === 0) && <p className="text-center text-sm text-gray-400 py-8">{t("start_conversation")}</p>}
            {messages?.map((m: any) => {
          ...
          <div className="p-3 border-t dark:border-gray-700 shrink-0">
            {chatPrice > 0 && activeConv?.creator_id !== user?.id && (
              <p className="text-[10px] text-yellow-600 dark:text-yellow-400 mb-1">💰 {t("credit_per_message")}: {chatPrice / 1000} Credit</p>
            )}
            <div className="flex gap-2">
              <Input value={message} onChange={e => setMessage(e.target.value)} placeholder={t("type_message")} className="flex-1"
    ...
    return (
      <AuthGuard><Navbar />
        <div className="mx-auto max-w-2xl p-4">
          <h1 className="text-lg font-bold mb-4">{t("new_chat")}</h1>
          <Card><CardContent className="p-4 space-y-3">
            <p className="text-sm text-gray-500 dark:text-gray-400">{t("send_first")}</p>
            {chatPrice > 0 && <p className="text-xs text-yellow-600 dark:text-yellow-400">💰 {t("each_message_costs")} {chatPrice / 1000} Credit</p>}
            <Input value={message} onChange={e => setMessage(e.target.value)} placeholder={t("type_message")}
              onKeyDown={e => { if (e.key === "Enter" && message.trim()) startChat.mutate(); }} />
            <Button onClick={() => startChat.mutate()} disabled={!message.trim() || startChat.isPending} className="w-full"><Send className="mr-2 h-4 w-4" /> {t("submit")}</Button>
          </CardContent></Card>
        </div>
      </AuthGuard>
    );
      ...
      <div className="mx-auto max-w-2xl p-4">
        ...
        {(!conversations || conversations.length === 0) && (
          <Card><CardContent className="p-8 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">{t("visit_creator")}</p>
          </CardContent></Card>
        )}
