  ...
  return (
    <AuthGuard>
      ...
      <main className="mx-auto max-w-2xl px-4 py-6 sm:py-8">
        <h1 className="mb-6 text-xl sm:text-2xl font-display font-black tracking-tight">{t.nav.wallet}</h1>
        ...
        <Card className="mb-6 bg-gradient-hero dark:bg-gradient-hero-dark border-0 text-white overflow-hidden relative">
          ...
          <CardContent className="p-6 sm:p-8 relative">
            <div className="flex items-center justify-between mb-4">
              <p className="text-primary-200 text-sm font-medium">{t.wallet.creditBalance}</p>
        ...
        {user?.role === "creator" && (
          <Link href="/dashboard/withdrawals" className="flex items-center gap-2 text-sm text-primary hover:underline font-medium mb-6">
            {t.wallet.withdrawToBank} <ArrowRight className="h-3 w-3" />
          </Link>
        )}

        <h2 className="mb-4 text-base font-bold">{t.wallet.transactionHistory}</h2>
        <div className="space-y-2.5">
          ...
          {txs?.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t.wallet.topupToStart}</p>
              <Link href="/wallet/topup"><Button variant="outline" size="sm" className="mt-4">{t.wallet.topupButton}</Button></Link>
            </div>
          )}
        </div>
      </main>
